...
class EpidemySuite extends FunSuite {
  ...
  test("movements") {
    ...
    val m1 = person.neighbors

    assert(m1.exists(_ == (0, 1)))
    assert(m1.exists(_ == (1, 0)))
    assert(m1.exists(_ == (7, 0)))
    assert(m1.exists(_ == (0, 7)))

    person.move((5, 5))

    val m2 = person.neighbors

    assert(m2.exists(_ == (5, 4)))
    assert(m2.exists(_ == (4, 5)))
    assert(m2.exists(_ == (6, 5)))
    assert(m2.exists(_ == (5, 6)))
