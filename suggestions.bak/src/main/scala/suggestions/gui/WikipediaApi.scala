...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = obs.map(text => text.replace(' ', '_'))
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = Observable { o =>
      obs.subscribe(
        value => o.onNext(Success(value)),
        { throwable =>
          o.onNext(Failure(throwable))
          o.onCompleted
        },
        () => o.onCompleted)
    }
